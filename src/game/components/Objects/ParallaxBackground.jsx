...
export const ParallaxBackground = ({ biomeTextures, towerTexturesLeft, towerTexturesRight, canvasSize, cameraY }) => {
    ...
    const RATIO_LARGEUR_JEU = isMobileView ? 0.96 : 0.428;
    ...
    const IMG_W = 1920;
    const IMG_H = 1080;
    const bgScaleByWidth = canvasSize.width / IMG_W;
    const bgScaleByHeight = canvasSize.height / IMG_H;
    const bgScale = Math.max(bgScaleByWidth, bgScaleByHeight);
    const bgScaledWidth = Math.ceil(IMG_W * bgScale);
    const bgScaledHeight = Math.ceil(IMG_H * bgScale);
    const bgOffsetX = Math.floor((canvasSize.width - bgScaledWidth) / 2);

    const LEFT_TOWER_WIDTH = offsetX;
    const RIGHT_TOWER_WIDTH = canvasSize.width - (offsetX + PLAY_AREA_WIDTH);

    const showTowers = !isMobileView && towerTexturesLeft?.length > 0 && LEFT_TOWER_WIDTH > 20;
    const refTowerTexture = towerTexturesLeft?.[0];
    const towerScale = showTowers ? LEFT_TOWER_WIDTH / refTowerTexture.width : 0;
    const towerScaledHeight = showTowers ? Math.floor(refTowerTexture.height * towerScale) : 1;
